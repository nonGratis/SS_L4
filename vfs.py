...
class VFS:
    ...
    def unlink(self, name: str) -> bool:
        ...
        if inode.hard_links == 0:
            file_is_open = any(
                of.inode == inode_num for of in self.fs.open_files.values()
            )
            if not file_is_open:
    ...
    def read(self, fd: int, size: int) -> Optional[bytes]:
        ...
        while bytes_read < actual_size:
            ...
            if block_num is None:
                ...
                bytes_to_read = min(
                    actual_size - bytes_read, self.fs.block_size - byte_offset
                )
    ...
    def write(self, fd: int, data: bytes) -> Optional[int]:
        ...
        while bytes_written < len(data):
            ...
            bytes_to_write = min(
                len(data) - bytes_written, self.fs.block_size - byte_offset
            )

            block_data[byte_offset : byte_offset + bytes_to_write] = data[
